#!/usr/bin/env node
/*
 * enrich-launch-day.mjs — Launch-day bonding-curve microstructure personas generator.
 *
 * Generates snipers, bundlers, dev-watchers, LP-puller paranoiacs, and migration-arb bots.
 * Carries exact existing pack pins: robinhood, retail-trading, doppler.
 *
 * Usage:
 *   node scripts/enrich-launch-day.mjs --dry
 *   node scripts/enrich-launch-day.mjs --seed launch-day-v1
 */
...
const SEED = String(arg('seed', 'launch-day-v1'));
const N_PROFILES = parseInt(arg('profiles', '1000'), 10);
const N_SCENARIOS = parseInt(arg('scenarios', '24'), 10);
const OUT = String(arg('out', 'outputs/enrich-launch-day'));
...
const dopplerPool = ARCHETYPES.filter((a) => a.tags.includes('doppler') || a.tags.includes('trading'));
if (!dopplerPool.length) throw new Error('empty archetype pool');

console.log(`[launch-day] seed=${SEED} profiles=${N_PROFILES} scenarios=${N_SCENARIOS} dry=${DRY}`);

// profiles
const profiles = [];
for (let i = 0; i < N_PROFILES; i++) {
  const arch = S.R.pick(rng, dopplerPool);
  const p = P.buildFullProfile(rng, { archetype: arch });
  // Pin exact existing pack tags: robinhood, retail-trading, doppler, launch-day
  p.content.tags = Array.from(new Set([...p.content.tags, 'robinhood', 'retail-trading', 'doppler', 'launch-day', 'sniper', 'bundler']));
  profiles.push(p);
}

// scenarios: trading / launch-day
...
for (let i = 0; i < N_SCENARIOS; i++) {
  ...
  scenarios.push({
    id: P.uuid(rng),
    slug: `${B.slugify(sc.title)}-launch-${BATCH_TAG}-${i}`.slice(0, 64),
...
const QA = {
  ...
  tagsPinned: ['robinhood', 'retail-trading', 'doppler', 'launch-day', 'sniper', 'bundler'],
};
...
const provParams = { seed: SEED, engine: 'psychosynth-synth-v4', batch: 'enrich-launch-day', authored: true };
const tHash = P.sha256('enrich-launch-day:' + SEED);
const files = {};

files['00_generation_run.sql'] = `-- enrich-launch-day batch — engine + run (idempotent).
...
VALUES (${sqlStr(RUN_ID)}, (SELECT id FROM generators WHERE slug=${sqlStr(GEN_SLUG)} AND version=${GEN_VER}), ${sqlStr(GEN_SLUG)}, ${GEN_VER}, ${sqlJson({ seed: SEED, batch: 'enrich-launch-day', profiles: N_PROFILES })}, ${sqlStr(MODEL)}, ${profiles.length + responses.length}, ${profiles.length + responses.length}, ${profiles.length + responses.length}, 'done', now())
ON CONFLICT (id) DO NOTHING;
`;
{
  const vals = scenarios.map((s) => `(${sqlStr(s.id)}, ${sqlStr(s.slug)}, ${sqlStr(s.category)}, ${sqlStr(s.title)}, ${sqlStr(s.description)})`);
  files['01_scenarios.sql'] = '-- launch-day trading scenarios (unique slugs).\n'
...
}
{
  ...
  files['02_profiles.sql'] = '-- Launch-day personas (approved).\n'
...
files['APPLY_ALL.sql'] = `-- enrich-launch-day batch. Run in order.
