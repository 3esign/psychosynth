#!/usr/bin/env node
/*
 * enrich-social-cascades.mjs — Social copy-trading cascades & follower-typology personas generator.
 *
 * Generates herd-cascade scenarios, follower-typology personas, and copy-trading reactions.
 * Uses clean new kebab-case tags: social-cascade, copy-trading, farcaster, meme-rotator.
 *
 * Usage:
 *   node scripts/enrich-social-cascades.mjs --dry
 *   node scripts/enrich-social-cascades.mjs --seed social-cascades-v1
 */
...
const SEED = String(arg('seed', 'social-cascades-v1'));
const N_PROFILES = parseInt(arg('profiles', '1000'), 10);
const N_SCENARIOS = parseInt(arg('scenarios', '24'), 10);
const OUT = String(arg('out', 'outputs/enrich-social-cascades'));
...
const socialPool = ARCHETYPES.filter((a) => a.domain === 'social' || a.tags.includes('social-trader') || a.tags.includes('trading'));
if (!socialPool.length) throw new Error('empty archetype pool');

console.log(`[social-cascades] seed=${SEED} profiles=${N_PROFILES} scenarios=${N_SCENARIOS} dry=${DRY}`);

// profiles
const profiles = [];
for (let i = 0; i < N_PROFILES; i++) {
  const arch = S.R.pick(rng, socialPool);
  const p = P.buildFullProfile(rng, { archetype: arch });
  // Add clean new kebab-case tags
  p.content.tags = Array.from(new Set([...p.content.tags, 'social-cascade', 'copy-trading', 'farcaster', 'meme-rotator']));
  profiles.push(p);
}
...
for (let i = 0; i < N_SCENARIOS; i++) {
  const sc = B.buildScenario(rng, 'social');
  scenarios.push({
    id: P.uuid(rng),
    slug: `${B.slugify(sc.title)}-soc-${BATCH_TAG}-${i}`.slice(0, 64),
...
const QA = {
  ...
  tagsPinned: ['social-cascade', 'copy-trading', 'farcaster', 'meme-rotator'],
};
...
const provParams = { seed: SEED, engine: 'psychosynth-synth-v4', batch: 'enrich-social-cascades', authored: true };
const tHash = P.sha256('enrich-social-cascades:' + SEED);
const files = {};

files['00_generation_run.sql'] = `-- enrich-social-cascades batch — engine + run (idempotent).
...
}
{
  ...
  files['02_profiles.sql'] = '-- Social cascade personas (approved).\n'
...
files['APPLY_ALL.sql'] = `-- enrich-social-cascades batch. Run in order.
