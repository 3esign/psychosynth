...
export default function PlaygroundPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans antialiased relative overflow-hidden pb-12">
      ...
      <div className="absolute bottom-[-10%] right-[-10%] w-[50%] h-[50%] bg-fuchsia-950/15 rounded-full blur-[120px]" />
      ...
      <header className="border-b border-slate-800/80 bg-slate-900/50 backdrop-blur sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <Link href="/lab/browse" className="text-sm text-slate-400 hover:text-slate-200 transition">
            Browse Corpus
          </Link>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-6 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <section className="lg:col-span-5 bg-slate-900/40 border border-slate-800/60 rounded-2xl p-6 backdrop-blur-md shadow-xl flex flex-col gap-6">
          <div>
            <h2 className="text-lg font-semibold text-slate-200">1. Adjust Psychometric Vector</h2>
            <p className="text-xs text-slate-400 mt-1">
              Customize the traits to feed into the conditioned agent\'s mental model.
            </p>
          </div>
          ...
          <div className="flex flex-col gap-4 border-b border-slate-800 pb-5">
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="grid grid-cols-12 gap-2 items-center">
                <span className="col-span-4 text-xs text-slate-300 font-medium" title="Loss Aversion Coefficient">
                  Lambda (\(\lambda\))
                </span>
                <input
                  type="range"
              ...
              {/* Alpha */}
              <div className="grid grid-cols-12 gap-2 items-center">
                <span className="col-span-4 text-xs text-slate-300 font-medium" title="Utility Exponent for Gains">
                  Alpha (\(\alpha\))
                </span>
                <input
                  type="range"
              ...
              {/* Beta */}
              <div className="grid grid-cols-12 gap-2 items-center">
                <span className="col-span-4 text-xs text-slate-300 font-medium" title="Utility Exponent for Losses">
                  Beta (\(\beta\))
                </span>
                <input
                  type="range"
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-4">
              <div className="flex justify-between items-center">
                <span className="text-xs text-slate-300 font-medium">Cognitive Mode</span>
              ...
              <div className="grid grid-cols-12 gap-2 items-center">
                <span className="col-span-4 text-xs text-slate-300 font-medium">CRT Score</span>
                <input
                  type="range"
                  min="0"
                  max="3"
                  step="1"
                  value={crtScore}
                  onChange={(e) => setCrtScore(parseInt(e.target.value))}
                  className="col-span-6 accent-cyan-500 h-1 bg-slate-800 rounded-lg appearance-none cursor-pointer"
                />
                <span className="col-span-2 text-right text-xs text-slate-400 font-mono text-cyan-300">
                  {crtScore} / 3
                </span>
              </div>
            </div>
        ...
        <section className="lg:col-span-7 flex flex-col gap-6">
          ...
          <div className="bg-slate-900/40 border border-slate-800/60 rounded-2xl p-6 backdrop-blur-md shadow-xl">
            <h2 className="text-lg font-semibold text-slate-200">2. Select Decision Scenario</h2>
            <div className="flex gap-2 mt-4 overflow-x-auto pb-2">
              {PREDEFINED_SCENARIOS.map((s, idx) => (
                <button
                  ...
                  className={`text-xs px-3 py-1.5 rounded-full border transition shrink-0 ${selectedScenarioIdx === idx ? 'bg-violet-500/20 text-violet-300 border-violet-500/40' : 'bg-slate-800/40 text-slate-400 border-slate-700/60 hover:text-slate-200'}`}
                >
                  {s.title}
                </button>
              ))}
              <button
                onClick={() => {
                  ...
                  setScenarioText(customScenario || 'Enter a custom situation...');
                }}
                className={`text-xs px-3 py-1.5 rounded-full border transition shrink-0 ${selectedScenarioIdx === -1 ? 'bg-violet-500/20 text-violet-300 border-violet-500/40' : 'bg-slate-800/40 text-slate-400 border-slate-700/60 hover:text-slate-200'}`}
              >
                Custom Scenario
              </button>
            </div>
            ...
            <div className="mt-4">
              <textarea
                ...
                rows={4}
                className="w-full bg-slate-950/80 border border-slate-800 rounded-xl p-3 text-sm text-slate-200 focus:outline-none focus:border-violet-500/60 font-sans resize-none"
                placeholder="Describe a situation that will test the agent\'s risk behavior and psychological profile..."
              />
            </div>
            ...
            <div className="mt-4 flex justify-end">
              <button
                ...
                className="px-6 py-2.5 rounded-xl font-semibold text-sm bg-gradient-to-r from-violet-500 to-fuchsia-600 hover:from-violet-600 hover:to-fuchsia-700 text-slate-100 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-violet-500/15 active:scale-[0.98] transition"
              >
          ...
          {/* Logs Terminal */}
          <div className="bg-slate-950 border border-slate-900 rounded-2xl p-4 font-mono text-xs flex flex-col gap-2 h-48 overflow-y-auto shadow-inner shadow-black/40">
            <div className="flex justify-between items-center text-[10px] text-slate-500 uppercase tracking-widest border-b border-slate-800 pb-2 mb-1">
              <span>x402 payment & settlement logger</span>
          ...
          {result && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* Control Agent */}
              <div className="bg-slate-900/30 border border-slate-800/40 rounded-2xl p-5 shadow-xl relative overflow-hidden">
                <div className="absolute top-0 left-0 w-1.5 h-full bg-slate-700" />
                <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider">Control Agent (Raw LLM)</h3>
                <div className="mt-4 flex flex-col gap-4">
                  <div>
                    <span className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold">Cognitive Reasoning</span>
                    <p className="text-xs text-slate-300 mt-1 leading-relaxed italic bg-slate-950/40 p-2.5 rounded-lg border border-slate-850">
                      {result.control?.reasoning}
                    </p>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold">Resulting Response</span>
              ...
              {/* Conditioned Agent */}
              <div className="bg-slate-900/35 border border-violet-900/30 rounded-2xl p-5 shadow-xl relative overflow-hidden">
                <div className="absolute top-0 left-0 w-1.5 h-full bg-gradient-to-b from-violet-500 to-fuchsia-600" />
                <div className="flex justify-between items-start">
                  <h3 className="text-sm font-bold bg-gradient-to-r from-violet-400 to-fuchsia-400 bg-clip-text text-transparent uppercase tracking-wider">
                    Conditioned Agent (Psychosynth)
                  </h3>
                  {result.conditioned?.emotional_arc && (
                    <span className="text-[10px] bg-fuchsia-500/10 text-fuchsia-300 border border-fuchsia-500/20 px-2 py-0.5 rounded-full font-mono">
                      {result.conditioned.emotional_arc}
                    </span>
                  )}
                </div>
                <div className="mt-4 flex flex-col gap-4">
                  <div>
                    ...
                    <p className="text-xs text-violet-100/90 mt-1 leading-relaxed italic bg-violet-950/20 p-2.5 rounded-lg border border-violet-900/20">
                      {result.conditioned?.reasoning}
                    </p>
                  </div>
                  <div>
                    <span className="text-[10px] text-fuchsia-400 uppercase tracking-wider font-semibold">Resulting Response</span>
